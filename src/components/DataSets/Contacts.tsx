...
const ContactRow = ({
  register, index, isFirst, removeRow, contactTypes, prefixFieldName = '',
}: ContactRowProps) => (
  <div className="flex-col sm:grid gap-2 grid-cols-4">
    ...
    <div className="flex items-center my-4 sm:my-0">
      ...
      <button
        type="button"
        className="float-right text-red-500 cursor-pointer sm:pb-2"
...
const Contacts = ({
  register, contacts = [], setValue, getValues, prefixFieldName = '',
}: ContactsProps) => {
  const { get } = useApi();
