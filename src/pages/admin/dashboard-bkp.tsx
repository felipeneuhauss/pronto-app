...
import { Meta, SaleEntity, SaleMapped, saleMapper } from 'shared/mappers'
...
const SaleRow = ({ sale }: {sale: SaleMapped}) => {
  ...
  return (
    <a href={`/admin/sale/${sale.id}`} className={`cursor-pointer border-l-2 border-l-solid block ${statusBorderColor}`}>
      ...
      : 'text-green-800 bg-green-200'}`
        ...
        <div className="w-full flex justify-between flex-wrap">
          <div className={'p-4 flex-column md:w-1/2'}>
            <div>
              <div>
            ...
            <div>
              ...
              <span className={'text-sm font-medium text-gray-900'}>{sale.customerBranch?.address?.street} - {sale.customerBranch?.address?.postalCode}</span>
            </div>
          </div>
          <div className={'p-4 flex-column md:w-1/2'}>
            <div>
            ...
            <div>
              ...
              <span className={'text-sm font-medium text-gray-900'}>{sale.supplier.name} </span>
            </div>
          </div>
        </div>
        <div className="grid grid-cols-2 p-4 gap-4 text-sm bg-gray-100 text-gray-700 sm:grid-cols-3">
          <div>
            <div className={'flex-column'}>
              <span className="text-xs font-bold inline-block w-full">Data da venda</span> {formatDate(sale.soldAt)}
            </div>
          </div>
          <div>
            <div className={'flex-column'}>
              <span className="text-xs font-bold inline-block w-full">Data da fatura</span> {formatDate(sale.invoiceDate)}
            </div>
          </div>
          <div>
            <div className={'flex-column'}>
              <span className="text-xs font-bold inline-block w-full">Data de vencimento</span> {formatDate(sale.expiresIn)}
            </div>
          </div>
          <div>
            <div className={'flex-column'}>
              <span className="text-xs font-bold inline-block w-full text-left">Pago em</span> {formatDate(sale.paidAt) || '-'}
            </div>
          </div>
          <div>
            <div className={'flex-column'}>
              <span className="text-xs font-bold inline-block w-full">Valor da venda</span> €{sale.saleValue || ''}
            </div>
          </div>
          <div>
            <div className={'flex-column'}>
              <span className="text-xs font-bold inline-block w-full">Valor faturado</span> €{sale.invoicedAmount || ''}
            </div>
          </div>
          <div>
            <div className={'flex-column'}>
          ...
          <div>
            <div className={`flex-column ${getDelayColor(sale)}`}>
    ...
    <AppLayout className="relative"
      header={
        <h2 className="font-semibold text-xl text-gray-800 leading-tight">
          <span className="text-right">{meta?.total}</span> vendas
        </h2>
      }>
      <div className="py-2">
