...
import { DateInput, Input, Select, SubTitle, Toggle, Uploader } from 'components/Form'
...
const Visit = ({
  ...
  return <>
    {<a className="float-right text-red-500 cursor-pointer pt-2"
      onClick={() => removeVisit(index)}><FontAwesomeIcon
        icon={faRemove} /> Remover</a>
    }
    <SubTitle>Visita {index + 1}</SubTitle>
    <div className="sm:grid grid-cols-3 gap-2">
      <DateInput label={'Data da visita'}
        name={`visits.${index}.createdAt`}
        register={register} required={true} />
      <Select label={'Situação da visita'} placeholder="Situação da visita"
        name={`visits.${index}.visitStatus`}
        required={true} register={register}
        options={visitStatusOptions} />
      <Input label={'Observação'} name={`visits.${index}.observation`}
        register={register} />
    </div>
  </>
}
...
const Sale = () => {
  ...
  const { register, handleSubmit, reset, setValue, getValues } = useForm<SaleMapped>({
    defaultValues: { }
  })
  ...
  const getSuggestions = (term: string): Promise<Option[]> => {
    return get(`/customers/${term}/auto-complete`)
  }
      ...
      <Card>
        <div className={'p-5'}>
          <form onSubmit={handleSubmit(onSubmit)} autoComplete="off">
            ...
            {sale?.id && <div className={`mb-4 p-4 text-sm text-shadow-md bg-gray-100 text-gray-700 rounded sm:grid grid-cols-2 gap-4 sm:grid-cols-3 ${getBorderStatusColor(sale)}`}>
              <div>
            ...
            <div className="mb-4">
              <Input label={'Código da fatura'} name={'invoice'}
                register={register} />
              {profile === ADMIN && <Select label={'Vendedor'} placeholder="Vendedor"
                name={'sellerId'} register={register} options={sellers} />}
              <Select label={'Representada'} placeholder="Representada"
                name={'supplierId'} required={true} register={register} options={suppliers} />
              <Autocomplete
            ...
            <div className="md:grid-cols-2 md:grid gap-2">
              <DateInput label={'Data da venda'} name={'soldAt'} required={true}
                register={register} />
              <Input label={'Valor da venda'} name={'saleValue'} required={true}
                register={register} mask={CURRENCY} />
              <DateInput label={'Data da fatura'} name={'invoiceDate'}
                register={register} />
              <Input label={'Valor da fatura'} name={'invoicedAmount'}
                register={register} mask={CURRENCY} />
              <DateInput label={'Data de vencimento'} name={'expiresIn'}
                register={register} />
              <DateInput label={'Data de pagamento'} name={'paidAt'} register={register} />
            </div>
            <Input label={'Motivo do atraso no pagamento'} name={'late_payment_motive'}
              register={register} />
            <div className={'py-2'}>
              ...
              <div className={'mt-4'}>
                {<Button variant={'dark'} type="button" className={'h-10'} onClick={addVisit}>
                  <FontAwesomeIcon icon={faAdd} /> Registar visita
                </Button>}
              </div>
            </div>
            <div className={'border-t-2'}>
              <div className={'flex-col flex md:flex-row py-4 gap-2'}>
                <Uploader label={'Anexar comprovante'}
              ...
              {newFileUploaded && <>
                <Toggle label={'Adicionar observação no e-mail?'} setValue={setValue}
                  name={'changeDefaultEmailContent'} handleChange={(value) => setChangeDefaultEmailContent(value)} />
                {changeDefaultEmailContent && <div>
                  <SubTitle>Adicionar observação</SubTitle>
                  <Input label={'Observação'} name="billingObservation"
                    register={register} />
                </div>}
              </>}
            </div>
