  ...
  <>
    <SubTitle>
      ...
      <a
        ...
        onClick={() => removeCustomerBranch(index)}
      >
        <FontAwesomeIcon
          icon={faRemove}
        />
...
const SupplierCode = ({
  ...
  >
    ...
    <div>
      <Input
        label="Código"
        name={`supplierCustomers[${index}].code`}
        register={register}
      />
    </div>
  </div>
...
const Customer = () => {
  ...
  useEffect(() => {
    ...
    get('/customers/activity-sectors')
      .then((data) => {
        setActivitySectors(data.map(({ id: activitySectorId, name } : ActivitySectorEntity) => ({
          value: activitySectorId,
          label: name,
        })));
      })
      .catch((error) => {
    ...
    get('/suppliers')
      .then((data) => {
        setSuppliers(data.map((item: SupplierEntity) => supplierMapper(item)));
      })
      .catch((error) => {
        if (error.response.status !== 409) throw error;
      });
  ...
  useEffect(() => {
    if (customer && suppliers) {
      customer.supplierCustomers = suppliers.map((supplier) => {
        const supplierCustomer = customer.supplierCustomers
          ?.find((item) => item.supplierId === supplier.id);
        if (supplierCustomer) {
          ...
          <form onSubmit={handleSubmit(onSubmit)} autoComplete="off">
            ...
            <div className="mb-4">
              ...
              <div className="my-3">
                ...
                <div className="mt-4">
                  <Button variant="dark" type="button" className="h-12" onClick={addCustomerBranch}>
